\section{Mathematical Notation}
	...
	The most basic mathematical word is a \emph{set}\index{set}.  
	...
	$\notin$ is used to specify that it is not.  For example,
	\[
	...
	to call elements of a set \emph{points}\index{point} regardless of whether or not you
	...
	There's something funny about that last example, though.  The two sets are not only subsets/supersets
	of each other, but they're also \emph{equal}.  As surprising as it seems, we actually need to define
	...
	\begin{example}
		...
		$x=m+1$ for some odd $m$ and so by the definition of "odd," $m=2k+1$
		for some whole number $k$.  Thus, 
		\begin{align*}
			x=m+1&=(2k+1)+1=2k+2\\
			&=2(k+1)=2n,
		\end{align*} where $n=k+1$. Therefore, $x\in A$.  Since $A\subseteq B$
	...
	\begin{example}
		...
		as well as zero).  To define $E$ as the even integers,
		we could write
		\[
	...
	\Set{-1,0,1,2,3}$.  Set unions and intersections are \emph{associative}, which means the placement of parentheses 
	does not matter in an expression involving only unions or intersections. 
	...
	is the set of all triples of real numbers.  If you take the Cartesian product of a set
	with itself some number of times, you can represent it with
	an exponent (this is something you've probably seen before, but with numbers).  Thus, $\R\times \R\times \R$ can 
	be written as $\R^3$, which is a set we've
	seen before%
	\footnote{
